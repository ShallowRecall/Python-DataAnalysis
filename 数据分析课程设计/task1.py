...
universityPart = ['Peking University', 'Tsinghua University', 'Zhejiang University', 'Shanghai Jiao Tong University',
                  'Nanjing University', 'Xi\'an Jiaotong University', 'China University of Technology',
...
plt.legend();
plt.title('Two of China\'s Top Universities')
...
for i in range(7, 9):
    ...
    for a, b in zip(year, scores[i].tolist()):
        plt.text(a, b, b, ha='center', va='bottom', fontsize=8)
plt.legend();
plt.title('Two of Wuhan\'s Top Universities')
x_major_locator = MultipleLocator(1)
y_major_locator = MultipleLocator(10)  # 把y轴的刻度间隔设置为20，并存在变量里
ax = plt.gca()
ax.xaxis.set_major_locator(x_major_locator)
...
plt.title('Total Numbers of Universitys Enrolled Per Province')
...
plt.title('The Average Admission Scores of Wuhan University in Each Province')
plt.xticks(range(1, len(province) + 1), province, rotation=90)
plt.ylabel('score')
plt.show()
