...
data2=pd.read_excel('/Users/zhouyu/PycharmProjects/pythonProject/数据分析课程设计/data/quanguoputonggaodengxuexiaomingdan.xls')
...
plt.title('各省大学占比')
plt.pie(arr,labels=label,autopct='%1.2f%%')
...
plt.legend(x1)
plt.title('学校类型占比')
plt.pie(y,labels=x1,autopct='%1.2f%%')
plt.legend(loc='upper right',fontsize=7,bbox_to_anchor=(1.1, 1.05))
plt.show()
...
plt.xlabel('办学层')
plt.ylabel('学校数量/个')
plt.bar(x1,y,width=0.8,align="center")
plt.plot(x1,y,'r-')
...
print(groupby_province1[groupby_province1['数量']>=7])

arr1=list(groupby_province1['数量'])
arr2=list(groupby_province1['数量'].drop_duplicates())
print(arr1)
print(arr2)

plt.title('各城市大学数量（仅显示数量多于平均值的）')
plt.hist(arr1,bins=45,color="red",edgecolor="black")
plt.show()

plt.title('各城市大学数量众数')
plt.hist(arr2,bins=45,color="red",edgecolor="black")
plt.show()
...
ax.set_xticks(np.arange(len(["本科", "专科"])))
ax.set_yticks(np.arange(len(count_labels)))
# ... and label them with the respective list entries
ax.set_xticklabels(["本科", "专科"])
ax.set_yticklabels(count_labels)
...
ax.set_title("数量分层和办学层级分布热力图")
fig.tight_layout()
plt.show()

data = np.array([[36, 18], [15, 16], [22, 11], [8, 5], [1, 5],[0,4],[0,3],[1,0]])
# fig, ax = plt.subplots()
plt.scatter(data[:,0], data[:,1], s=data[:,1]*10)
plt.xlabel('专科')
plt.ylabel('本科')
plt.title("数量分层和办学层级分布气泡图")
plt.tight_layout()
plt.show()
